"""
Server Side Events (SSE) async client for Python.
...
class SSEClient(object):
    ...
    It is inspired by https://github.com/mpetazzoni/sseclient.
    """

    def __init__(self, event_stream, char_enc='utf-8'):
        """Initialize the SSE client over an existing, ready to consume
        event stream aiohttp.StreamReader.
        """
        self._logger = logging.getLogger(self.__class__.__module__)
        self._logger.debug('Initialized async SSE client from event source.')
    ...
    async def _read(self):
        ...
        data = b''
        async for line in self._event_stream:
            data += line
