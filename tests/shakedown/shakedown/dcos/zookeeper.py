...
def get_zk_node_data(node_name):
    znode_url = f"{dcos_url()}/exhibitor/exhibitor/v1/explorer/node-data?key={node_name}"
...
def get_zk_node_children(node_name):
    znode_url = (
        f"{dcos_url()}/exhibitor/exhibitor/v1/explorer/node?key={node_name}"
    )
...
def delete_zk_node(node_name):
    znode_url = f"{dcos_url()}/exhibitor/exhibitor/v1/explorer/znode/{node_name}"
    auth = DCOSAcsAuth(dcos_acs_token())
